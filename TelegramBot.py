...
def reply(input_message):
    ...
    global input_time
    global input_note
    global delete_on_date
    global delete_on_time
    global show_on_date
    global show_on_time
    global show_on_date_input
    global show_on_time_input
    ...
    if input_message.text == 'Начать' or input_message.text == 'Вернуться в начало':
    ...
    elif delete_time is True:
        if re.fullmatch(r'\d{2}:\d{2}', input_message.text):
            if check_time(input_message.text) is True:
                delete_time = False
                delete_on_time = input_message.text
                delete_note(delete_on_date, delete_on_time)
    ...
    elif show_on_date is True:
        show_on_date = False
        if re.fullmatch(r'\d{2}.\d{2}.\d{4}', input_message.text):
            if check_date(input_message.text) is True:
                show_on_date_input = input_message.text
                show_on_time = True
                get_back()
            else:
                date_bool = True
            ...
            bot.send_message(input_message.chat.id, 'Введенная дата не соответствует формату дд.мм.ггг! '
                                                    '\nПовторите попытку')
    elif show_on_time is True:
        show_on_time = False
        if re.fullmatch(r'\d\d:\d\d', input_message.text):
            if check_time(input_message.text) is True:
                show_on_time_input = input_message.text
                if get_count_notes_on_date(show_on_date_input, show_on_time_input) != 0:
                    show_send = show_on(show_on_date_input, show_on_time_input)
    ...
    elif date_bool is True:
        date_bool = False
        if re.fullmatch(r'\d{2}.\d{2}.\d{4}', input_message.text):
            if check_date(input_message.text) is True:
                input_date = input_message.text
                time_bool = True
                get_back()
            else:
                date_bool = True
            ...
            bot.send_message(input_message.chat.id, 'Введенная дата не соответствует формату дд.мм.ггг! '
                                                    '\nПовторите попытку')
            date_bool = True
    elif time_bool is True:
        time_bool = False
        if re.fullmatch(r'\d\d:\d\d', input_message.text):
            if check_time(input_message.text) is True:
                input_time = input_message.text
                if get_count_notes_on_date(input_date, input_time) != 1:
                    note_bool = True
        ...
        done_note = input_db(input_date, input_time, input_note)
